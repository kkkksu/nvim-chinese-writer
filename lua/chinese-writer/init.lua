...
local function enable_im_switch(cfg)
  ...
  im_group = vim.api.nvim_create_augroup("ChineseWriterIM", { clear = true })
...
local function disable_im_switch()
  if im_group then
    pcall(vim.api.nvim_del_augroup_by_id, im_group)
    im_group = nil
  end
end
...
local function enable_punct_map()
  for _, m in ipairs(punct_mappings.nv) do
  ...
  for _, m in ipairs(punct_mappings.cmd) do
    vim.cmd(string.format("cmap <silent> %s %s", m[1], m[2]))
  end
end

local function disable_punct_map()
  for _, m in ipairs(punct_mappings.nv) do
    pcall(vim.cmd, "nunmap " .. m[1])
    pcall(vim.cmd, "vunmap " .. m[1])
    pcall(vim.cmd, "ounmap " .. m[1])
  end
  for _, m in ipairs(punct_mappings.cmd) do
    pcall(vim.cmd, "cunmap " .. m[1])
  end
end
...
local function enable_pinyin_jump()
...
local function disable_pinyin_jump()
  for _, mode in ipairs({ "n", "x", "o" }) do
...
function M.enable()
  ...
  enable_im_switch(M.config.im_switch)
  enable_punct_map()
  enable_pinyin_jump()
  M.enabled = true
...
function M.setup(opts)
  ...
  -- 如果配置中某项显式 enabled，启动时自动开启
  if M.config.im_switch.enabled or M.config.punct_map.enabled or M.config.pinyin_jump.enabled then
    M.enable()
  end
